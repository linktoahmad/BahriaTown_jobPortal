...
router.get("/jobs", jwtAuth, (req, res) => {
  ...
  if (req.query.NumberOfResults) {
    ...
    arr = [...arr, { $limit: parseInt(req.query.NumberOfResults) }];
  }
...
//jobs updates on landing page
router.get("/jobsUpdate", (req,res) => {
  let sortParams = { dateOfPosting: -1 };
  let arr=[{ $sort: sortParams }]

  Job.aggregate(arr)
    .then((posts) => {
      if (posts == null) {
        res.status(404).json({
          message: "No new jobs found found",
        });
        return;
      }
      res.json(posts);
  ...
  if (user.type == "recruiter") {
    ...
    JobApplicant.findOne({ userId: user._id })
      .then((jobApplicant) => {
        ...
        if (data.MyAddress) {
          jobApplicant.MyAddress = data.MyAddress;
        }
...
router.put("/applications/:id", jwtAuth, (req, res) => {
  ...
  const interviewdate = req.body.dateOfInterview;
  const medium = req.body.InterviewMedium;
      ...
      Application.findOneAndUpdate(
        ...
        },
        {
          $set: {
            ...
            reasonOfRejection:req.body.reasonOfRejection!==""?req.body.reasonOfRejection:""
          },
        }
      )
        .then((application) => {
